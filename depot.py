...
def add_user():
	...
	if _name and _email and _password and request.method == 'POST':
		...
		result = jsonify({'$oid': str(id.inserted_id) })
		resp = result
	...
	if _type == 'buy':
		...
		if _id and request.method == 'PUT':
			...
			if exist.count() == 0:
				...
				resp = jsonify({'$oid': str(result.upserted_id)})
				resp.status_code = 200
				return resp
				...
				if total_amount > _amount:
					for each in query_results[0]['stock']:
						...
						if available_shares_depot > shares_to_sell:
							...
							resp = jsonify({'$oid': str(query.upserted_id)})
							resp.status_code = 200
							return resp
